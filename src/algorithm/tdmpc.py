...
class TOLD(nn.Module):
	...
	def __init__(self, cfg):
		...
		self._pi = h.actor(cfg.latent_dim, cfg.mlp_dim, cfg.action_dim)
	...
	def pi(self, z, std=0):
		...
		# return mu
		action, log_prob, mu = self._pi.sample(z)
...
class TDMPC():
	...
	def plan(self, obs, eval_mode=False, step=None, t0=True):
		...
		if num_pi_trajs > 0:
			...
			for t in range(horizon):
				pi_actions[t], _, _ = self.model.pi(z, self.cfg.min_std)
	...
	def update_pi(self, zs, obs, step):
		...
		# z = self.model.h(obs)
		# policy_mu, policy_std = self.model._pi(z)
		# pi_loss += -0.5 * (torch.log(policy_std**2/(std.detach())**2) - \
		# 				  policy_std**2/(std.detach())**2 - (policy_mu - mu.detach())**2/(std.detach())**2 + 1).mean()
	...
	def _td_target(self, next_obs, reward):
		...
		next_action, next_log_prob, _ = self.model.pi(next_z, self.cfg.min_std)
		td_target = reward + self.cfg.discount * \
			(torch.min(*self.model_target.Q(next_z, next_action)) - self.alpha * next_log_prob)
		return td_target
	...
	def update(self, replay_buffer, step):
		...
		pi_loss = self.update_pi(zs, obs, step)
